...
export const GET = route(
  ...
  async () => {
    ...
    if (!price.available) {
      // Public read endpoints — return 200 with availability sentinel so
      // pollers (counter, landing) don't generate console noise on
      // pre-launch dev. Game flow checks `available` before letting players
      // create an entry.
      return ok({
        available: false,
        reason: price.reason,
